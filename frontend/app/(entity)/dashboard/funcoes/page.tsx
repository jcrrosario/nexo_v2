...
export default function FuncoesPage() {
  const [dados, setDados] = useState<Funcao[]>([])
  ...
  const [confirmar, setConfirmar] = useState<Funcao | null>(null)
  const [log, setLog] = useState<Funcao | null>(null)
  ...
  async function carregar() {
    const res = await api.get(
      `/entity/funcao?page=${page}&limit=${pageSize}&search=${busca}`,
    )
  ...
  async function salvar() {
    if (editando) {
      await api.put(`/entity/funcao/${form.func_id}`, form)
    } else {
      await api.post('/entity/funcao', form)
    }
    setOpen(false)
    carregar()
  }
  ...
  async function excluir() {
    if (!confirmar) return
    await api.put(`/entity/funcao/${confirmar.func_id}/excluir`, {})
    setConfirmar(null)
    carregar()
  }
  ...
  function gerarExcel() {
    ...
    XLSX.writeFile(wb, 'funcoes.xlsx')
  }
  ...
  return (
    ...
    >
      ...
      {open && (
        <div style={overlay}>
          <div style={modal}>
            <h3>{editando ? 'Editar' : 'Nova'} Função</h3>
